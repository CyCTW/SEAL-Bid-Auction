...
const verifyStageTwoNZIKProof = (proof, groups, publics) => {
  ...
  const verify_6 = () => {
    ...
    const right =
      (proof.commitment_13_ * pow(L * egcd(g, p), -proof.challange_1, p)) % p;
...
const init = ({
  ...
  if (statement === 0n) {
    // current bit=1 and last deciding bit=1
    M = pow(R, x, p);
    M_ = pow(R_, x_, p);
    L = pow(g, a * b + 1n, p);
  } else if (statement === 1n) {
    // current bit=0 and last deciding bit=1
    M = pow(Y, x, p);
    M_ = pow(R_, x_, p);
    L = pow(g, a * b, p);
  } else if (statement === 2n) {
    // current bit=0 and last deciding bit=0
    M = pow(Y, x, p);
    M_ = pow(Y_, x_, p);
    L = pow(g, a * b, p);
  }
